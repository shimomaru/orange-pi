...
while true; do
    ...
    if [[ "$duty" =~ ^[0-9]+$ ]] && [ "$duty" -ge 0 ] && [ "$duty" -le 100 ]; then
        ...
        # Wait for 2.5 seconds before starting the decrease
        echo "Waiting for 2.5 seconds before starting to decrease the duty cycle..."
        sleep 2.5

        # Gradually decrease the duty cycle at 1% per millisecond down to 1%
        current_duty=$duty
        while [ $current_duty -gt 1 ]; do
            # Reduce the duty cycle by 1% per millisecond
            current_duty=$((current_duty - 1))
            set_duty_cycle $current_duty
            sleep 0.001  # Sleep for 1 millisecond
        done
        echo "Duty cycle reduced to 1%."
    else
